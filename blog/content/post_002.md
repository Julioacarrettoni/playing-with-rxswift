...
Now we need to call this method once, a good place could be [here](https://github.com/Julioacarrettoni/playing-with-rxswift/blob/master/002/Before/RxPlaying/SceneDelegate.swift#L12) in the `SceneDelegate.swift` file.
...
But there is a better way, the RxWay. All observables have a very useful operator called [do](https://github.com/ReactiveX/RxSwift/blob/70b8a33c5c3f4c3b15ebf10b638d2b15cfafb814/RxSwift/Traits/Single.swift#L157-L170), and this one is a big one:
...
That's up to you, I won't do it in order to keep the code as vanilla as possible to avoid confusions.  
And with that we so how easy is too add logs to our code while still keeping some separation between business logic and logs / analytics.  
...
|-|
#### Introduces new unit tests
It's time to add some unit tests, the question now is "_What should we test?_" As many tests we write the better, but writing tests just for the sake of it is not good.  
...
```
No, I'm not being **too** far fetched or pessimist, you think it is impossible for someone to come and remove that very important line, well in this context it seems highly unlikely given the simplicity of the program, but even on the most simple logics someone making a big refactor and solving a rebase with lots conflicts might get confused and solve it the wrong way, it can happen (I have seen it MANY times) and if this situation happens it would be terrible as neither of the logs we have will catch it.  
...
Now find the `RxPlayingTests.swift` file, get ride of the `testPerformanceExample` function as we won't use it and rename `testExample` to something more representative, but remember it has to always be prefixed with the word `test`, take no arguments and return void<sup>[6](#6)</sup>. I'm very bad at naming things (Take the blog or the fake company as good examples) but a convention is `what-is-being-tested`+`the-state-change`+`expected-value` so maybe `testSystemSingleWhenFailsReturnsError` is a suitable name.
...
First let's import the Target we want to test with the `@testable` attribute<sup>[7](#7)</sup> so we have access to it's internal members and also the FakeService module as we will use it.
```Swift
@testable import RxPlaying
import FakeService
```

In real life we would need to introduce some dependency injection mechanism so we can switch the FakeService with a MockService we can control and force it to return `nil` on demand and avoid making network calls, unit tests that makes network calls are not unit tests, no matter what arguments you use they are all wrong to me. Luckily to us FakeService comes with mocking mechanism (is a Mock of a service after all).    
...
Test are build to fail and you HAVE to make sure they fail at least when you expect them to fail, otherwise the test is pointless. (Also that code if you are following the post on your Xcode that code is emitting warnings, so there is something fishy with it to say the least)
...
But if we run the test again it won't fail.

You can play putting breakpoints and check that when `failNext` is set to `true` our `XCTAssert(true)` is indeed executed but when `failNext` is set to `false` it is not, so why is the test passing? Easy, if it doesn't fail it passes, test are intended to check for failure conditions, that's why we have `XCTFail()` but not `XCTSuccess()` method.  
...
```
And if you are lucky enough your test may pass, but it won't always, there is a race condition and you might end up with and exception "_Parameter "test" must not be nil._" which basically happens when a test fails after it had finished running.  
...
Let's improve the test a little, first let's get ride of the warning by creating a dispose bag and retaining the returned value of `susbcribe`. In this case we can do it locally as the `wait` will hold the execution and the current scope won't end before time disposing of our bag and it's content.  
...
Also, it is a good idea to "fail fast", it doesn't look like much but if this test fails, it fails after 0.1 second, if you have 10 tests failing that's at least 1 second of extra delay, if you have hundreds, the test suit could be delayed a lot when there is a change breaking multiple tests and running CI costs money baby.  
...
And remember, if you don't want people nitpicking your PRs just make one with over 100 lines of codes, nobody really review those… Just kidding don't be an a*shole, your coworkers know where you sit.  
